#!/usr/bin/env python3
"""
Genera proyectos_legislativos.html embebiendo proyectos_data.json en template.html.

Uso:
    python3 data_builder.py     # regenera proyectos_data.json a partir del Excel + fuentes/*.md
...
def main():
    ...
    parser.add_argument("--data", default=HERE / "proyectos_data.json")
    parser.add_argument("--template", default=HERE / "template.html")
    parser.add_argument("--fonts-dir", default=HERE / "fonts")
    parser.add_argument("--out", default=HERE / "proyectos_legislativos.html")
    ...
    print(f"OK: {out_path} generado ({len(html):,} bytes, {data['total']} proyectos)")
